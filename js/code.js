		...
		xhr.onreadystatechange = function () {
			if (this.readyState == 4 && this.status == 200) {
				...
				for (let i = 0; i < jsonObject.results.length; i++) {
					...
					contactList += `</strong><div class='d-grid gap-2 d-md-flex justify-content-md-end'><button type='button' id='updateButton' class='btn btn-outline-primary' onclick='setUpPrev(${JSON.stringify(
						str,
					)});'>Update</button>`;
					contactList += `<button type='button' id='deleteButton' class='btn btn-outline-danger' onclick='deleteContact(${JSON.stringify(
						str,
					)});'>Delete</button>`;
...
// 	let tmp = '(' + str[0] + str[1] + str[2] + ')' + str[3] + str[4] + str[5] + '-'
