		...
		xhr.onreadystatechange = function () {
			if (this.readyState == 4 && this.status == 200) {
				...
				for (let i = 0; i < jsonObject.results.length; i++) {
					...
					contactList += `<button type='button' id='updateButton' class='btn btn-outline-primary ms-1' onclick='setUpPrev(${JSON.stringify(
						str,
					)});'>Update</button>`;
					contactList += `<button type='button' id='deleteButton' class='btn btn-outline-primary ms-1' onclick='deleteContact(${JSON.stringify(
						str,
					)});'>Delete</button>`;
					contactList +=
						"</div></div><span id='contactDeleteResult'></span></div>";
