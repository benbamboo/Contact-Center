		...
		xhr.onreadystatechange = function () {
			if (this.readyState == 4 && this.status == 200) {
				...
				for (let i = 0; i < jsonObject.results.length; i++) {
					...
					contactList += `<div class='col'><div class='card mb-3'><div class='card-body d-flex bd-highlight' data-bs-toggle='collapse' data-bs-target='#collapse${i}' aria-expanded='false' aria-controls='collapse${i}'><strong class='me-auto p-2 bd-highlight'>`;
					contactList += str['FirstName'] + ' ' + str['LastName'];
					// add update/delete button for each jsonObject.results[i]
					contactList += `</strong><div class='d-grid gap-2 d-md-flex justify-content-md-end'><button type='button' id='updateButton' class='btn btn-outline-primary p-2 bd-highlight' onclick='setUpPrev(${JSON.stringify(
						str,
					)});'>Update</button>`;
					contactList += `<button type='button' id='deleteButton' class='btn btn-outline-danger p-2 bd-highlight' onclick='deleteContact(${JSON.stringify(
						str,
					)});'>Delete</button>`;
					...
					for (let j = 2; j < 5; j++) {
						...
						if (j == 2) {
							contactList +=
								"<dt class='col-sm-3'>Email</dt><dd class='col-sm-9'>" +
								str[Object.keys(str)[j]] +
								'</dd>';
						} else {
							contactList +=
								"<dt class='col-sm-3'>Phone</dt><dd class='col-sm-9'>" +
								formatPhoneNumber(str[Object.keys(str)[j]]) +
								'</dd>';
						}
					}
